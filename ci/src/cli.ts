...
async function runBuildMode(argv: string[]): Promise<number> {
  ...
  const repoRoot = execSync("bash -c pwd").toString().trim();
  ...
  const getDirs = (names: Set<string>): Set<string> => {
    ...
    return new Set(
      stdoutArray(execSync(`bash -c "pnpm $(cat ${tmp}) exec bash -c pwd"`).toString()).map((d) =>
  ...
  const allPackageDirs = new Set(
    stdoutArray(execSync(`bash -c "pnpm -F !${ROOT_PKG_NAME}... exec bash -c pwd"`).toString()).map((d) =>
      toRepoRel(d, repoRoot)
    )
  );
  ...
  const changedSourcePaths = stdoutArray(
    execSync(
      `bash -c "git diff --name-only ${baseSha} ${headSha} -- ${toGitDiffExcludePathspec(nonSource)}"`
    ).toString()
  );
  ...
  const changedPackages: Array<{ path: string; name: string }> = JSON.parse(
    execSync(`bash -c "turbo ls --filter='[${baseSha}...${headSha}]' --output json"`).toString()
  ...
  for (let i = 0; i < changedPackageNames.length; i += 50) {
    ...
    const out = JSON.parse(
      execSync(`bash -c "turbo ls ${chunk.map((n) => `-F '...${n}'`).join(" ")} --output json"`).toString()
    );
